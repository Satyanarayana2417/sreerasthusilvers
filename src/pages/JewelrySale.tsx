...
import { Star, Heart, ShoppingBag, Check, X, Package, Tag } from "lucide-react";
...
const JewelrySale = () => {
  ...
  useEffect(() => {
    console.log('JewelrySale: Setting up real-time listener...');
    setLoading(true);
    
    const unsubscribe = subscribeToProductsBySubcategory('Jewelry', (fbProducts) => {
      const uiProducts = adaptFirebaseArrayToUI(fbProducts);
      const saleProducts = uiProducts.filter(product => product.oldPrice && product.oldPrice > 0);
          ...
          <div className="relative z-20 text-center px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex items-center justify-center gap-2 text-sm md:text-base text-white/90">
                ...
                <span>{'>'}</span>
                <span className="text-white font-medium">JEWELRY SALE</span>
              </div>
            </motion.div>
          </div>
        </section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                  {loading ? (
                    ...
                    <div className="col-span-full py-16 text-center">
                      <Tag className="w-20 h-20 mx-auto mb-4 text-muted-foreground" />
