...
const RoseGoldJewelry = () => {
  ...
  useEffect(() => {
    console.log('RoseGoldJewelry: Setting up real-time listener...');
    setLoading(true);
    
    const unsubscribe = subscribeToProductsBySubcategory('Jewelry', (fbProducts) => {
      const uiProducts = adaptFirebaseArrayToUI(fbProducts);
      const roseGoldJewelry = uiProducts.filter(product => {
        const lowerTitle = product.title.toLowerCase();
        return lowerTitle.includes('rose gold') || lowerTitle.includes('rose-gold');
      });
      setProducts(roseGoldJewelry);
      
      if (roseGoldJewelry.length > 0) {
        const prices = roseGoldJewelry.map(p => p.price);
          ...
          <div className="relative z-20 text-center px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex items-center justify-center gap-2 text-sm md:text-base text-white/90">
                ...
                <span>{'>'}</span>
                <span className="text-white font-medium">ROSE GOLD JEWELRY</span>
              </div>
            </motion.div>
          </div>
        </section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                  {loading ? (
                    ...
                    <div className="col-span-full py-16 text-center">
                      ...
                      <h3 className="text-xl font-semibold mb-2">No Rose Gold Jewelry Available</h3>
...
export default RoseGoldJewelry;
