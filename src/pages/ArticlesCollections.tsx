...
const ArticlesCollections = () => {
  ...
  // Fetch all products from Firebase and filter by articles-related categories
  useEffect(() => {
    setLoading(true);
    const unsubscribe = subscribeToProducts(
      (fbProducts) => {
        const uiProducts = adaptFirebaseArrayToUI(fbProducts);
        // Filter only articles-related products
        const articlesProducts = uiProducts.filter(product => {
          // Check if category contains articles-related keywords
          const categoryLower = (product.category || '').toLowerCase();
          return categoryLower.includes('article') || 
                 categoryLower.includes('pooja') || 
                 categoryLower.includes('kalash') || 
                 categoryLower.includes('coconut') || 
                 categoryLower.includes('footwear') || 
                 categoryLower.includes('gopuram') ||
                 categoryLower.includes('camel') ||
                 categoryLower.includes('jhula');
        });
        setProducts(articlesProducts);
        setLoading(false);
      },
      true // activeOnly
    );
      ...
      <main>
        ...
        <div className="md:hidden sticky top-0 z-50 bg-white border-b border-gray-200">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <h1 className="text-lg font-semibold text-gray-900" style={{ fontFamily: "'Poppins', sans-serif" }}>Articles Collections</h1>
          </div>
        </div>
