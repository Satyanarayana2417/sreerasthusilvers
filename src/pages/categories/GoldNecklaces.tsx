...
const GoldNecklaces = () => {
  ...
  useEffect(() => {
    console.log('GoldNecklaces: Setting up real-time listener...');
    setLoading(true);
    ...
    const unsubscribe = subscribeToProductsBySubcategory('Necklaces', (fbProducts) => {
      ...
      const goldNecklaces = uiProducts.filter(product => 
        product.title.toLowerCase().includes('gold') ||
        (product as any).material?.toLowerCase().includes('gold')
      );
      setProducts(goldNecklaces);
      
      if (goldNecklaces.length > 0) {
        const prices = goldNecklaces.map(p => p.price);
          ...
          <div className="relative z-20 text-center px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex items-center justify-center gap-2 text-sm md:text-base text-white/90">
                ...
                <span>{'>'}</span>
                <span className="text-white font-medium">GOLD NECKLACES</span>
              </div>
            </motion.div>
          </div>
        </section>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              {loading ? (
                ...
                <div className="col-span-full py-16 text-center">
                  ...
                  <h3 className="text-xl font-semibold mb-2">No Gold Necklaces Available</h3>
