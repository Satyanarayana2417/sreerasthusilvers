...
const PearlNecklaces = () => {
  ...
  useEffect(() => {
    console.log('PearlNecklaces: Setting up real-time listener...');
    setLoading(true);
    ...
    const unsubscribe = subscribeToProductsBySubcategory('Necklaces', (fbProducts) => {
      ...
      const pearlNecklaces = uiProducts.filter(product => 
        product.title.toLowerCase().includes('pearl') ||
        (product as any).material?.toLowerCase().includes('pearl')
      );
      setProducts(pearlNecklaces);
      
      if (pearlNecklaces.length > 0) {
        const prices = pearlNecklaces.map(p => p.price);
      ...
      <main>
        <section className="relative h-[200px] md:h-[300px] flex items-center justify-center overflow-hidden">
          <div className="absolute inset-0 z-0">
            ...
            <img 
              src="https://images.unsplash.com/photo-1506630448388-4e683c67ddb0?w=1600&q=80" 
              alt="Pearl Necklaces"
          ...
          <div className="relative z-20 text-center px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex items-center justify-center gap-2 text-sm md:text-base text-white/90">
                ...
                <span>{'>'}</span>
                <span className="text-white font-medium">PEARL NECKLACES</span>
              </div>
            </motion.div>
          </div>
        </section>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              {loading ? (
                ...
                <div className="col-span-full py-16 text-center">
                  ...
                  <h3 className="text-xl font-semibold mb-2">No Pearl Necklaces Available</h3>
