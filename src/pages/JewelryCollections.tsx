...
const JewelryCollections = () => {
  ...
  // Fetch all products from Firebase and filter by jewelry-related categories
  useEffect(() => {
    setLoading(true);
    const unsubscribe = subscribeToProducts(
      (fbProducts) => {
        const uiProducts = adaptFirebaseArrayToUI(fbProducts);
        // Filter only jewelry-related products
        const jewelryProducts = uiProducts.filter(product => {
          // Check if category contains jewelry-related keywords
          const categoryLower = (product.category || '').toLowerCase();
          return categoryLower.includes('jewelry') || 
                 categoryLower.includes('jewellery') || 
                 categoryLower.includes('necklace') || 
                 categoryLower.includes('ring') || 
                 categoryLower.includes('bracelet') || 
                 categoryLower.includes('anklet') ||
                 categoryLower.includes('pendant') ||
      ...
      <main>
        ...
        <div className="md:hidden sticky top-0 z-50 bg-white border-b border-gray-200">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <h1 className="text-lg font-semibold text-gray-900" style={{ fontFamily: "'Poppins', sans-serif" }}>Jewelry Collections</h1>
          </div>
        </div>
