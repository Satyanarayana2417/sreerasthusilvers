...
const RoyalSilverChairs = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = subscribeToProductsBySubcategory('Royal Silver Chairs', (fbProducts) => {
            ...
            <div className="md:hidden">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  <span className="mx-1">/</span>
                  <span className="text-foreground">Royal Silver Chairs</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {loading ? (
                ...
                <div className="col-span-full py-16 text-center">
                  ...
                  <h3 className="text-xl font-semibold mb-2">No Chairs Available</h3>
                ...
                <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="relative bg-muted rounded-xl overflow-hidden aspect-square mb-4">
                    ...
                    <div className="absolute top-3 right-3 md:opacity-0 md:translate-y-2 md:group-hover:opacity-100 md:group-hover:translate-y-0 transition-all duration-300">
                      <motion.button onClick={(e) => { e.stopPropagation(); toggleWishlist(product.id, product.title); }}
                        whileTap={{ scale: 0.9 }} className={`p-2 rounded-full transition-all duration-300 ${wishlist.includes(product.id) ? "text-red-500" : "text-white/90 hover:text-red-500"}`}
          ...
          <div className="fixed bottom-6 right-6 z-[200] flex flex-col gap-3">
            {toasts.map((toast) => (
              <motion.div key={toast.id} initial={{ opacity: 0, x: 100, scale: 0.9 }} animate={{ opacity: 1, x: 0, scale: 1 }}
                ...
                <button onClick={() => removeToast(toast.id)} className="flex-shrink-0 p-1 hover:bg-black/10 rounded transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </motion.div>
            ))}
...
export default RoyalSilverChairs;
