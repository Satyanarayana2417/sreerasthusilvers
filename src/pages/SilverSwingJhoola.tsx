...
interface CartItem { product: UIProduct; quantity: number; }
interface Toast { id: string; message: string; type: 'success' | 'error' | 'info'; productTitle?: string; }

const SilverSwingJhoola = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = subscribeToProductsBySubcategory('Silver Swing (Jhoola)', (fbProducts) => {
  ...
  const { minPrice, maxPrice, priceRange } = useMemo(() => {
    ...
    return { minPrice: min, maxPrice: max, priceRange: [min, max] as [number, number] };
  }, [products]);
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      const matchesPrice = product.price >= localPriceRange[0] && product.price <= localPriceRange[1];
  ...
  const sortedProducts = useMemo(() => {
    ...
    switch (sortBy) {
      case 'price-low-high': return productsCopy.sort((a, b) => a.price - b.price);
      case 'price-high-low': return productsCopy.sort((a, b) => b.price - a.price);
      case 'newest': return productsCopy.sort((a, b) => b.id.localeCompare(a.id));
      case 'best-rating': return productsCopy.sort((a, b) => b.rating !== a.rating ? b.rating - a.rating : b.reviews - a.reviews);
            ...
            <div className="md:hidden">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  <a href="#" className="hover:text-primary transition-colors">Furniture</a>
                  <span className="mx-1">/</span>
                  <span className="text-foreground">Silver Swing Jhoola</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
              {loading ? Array.from({ length: 8 }).map((_, index) => (
                ...
                <div className="col-span-full py-16 text-center">
                  ...
                  <h3 className="text-xl font-semibold mb-2">No Swing Items Available</h3>
                  <p className="text-muted-foreground">Check back soon for new products!</p>
                </div>
              ) : sortedProducts.map((product, index) => (
                <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: index * 0.05 }} className="group cursor-pointer" onClick={() => handleProductClick(product.id)}>
                  <div className="relative bg-muted rounded-xl overflow-hidden aspect-square mb-4">
                    ...
                    <div className="absolute top-3 right-3 md:opacity-0 md:translate-y-2 md:group-hover:opacity-100 md:group-hover:translate-y-0 transition-all duration-300">
                      <motion.button onClick={(e) => { e.stopPropagation(); toggleWishlist(product.id, product.title); }} whileTap={{ scale: 0.9 }}
                        className={`p-2 rounded-full transition-all duration-300 ${wishlist.includes(product.id) ? "text-red-500" : "text-white/90 hover:text-red-500"}`}
                        style={{ textShadow: '0 1px 3px rgba(0,0,0,0.3)', filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.3))' }}>
                    ...
                    <div className="hidden md:block absolute bottom-3 left-3 right-3 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                      <motion.button onClick={(e) => { e.stopPropagation(); addToCart(product); }} whileTap={{ scale: 0.95 }}
                  ...
                  <div className="space-y-2 text-center">
                    ...
                    <div className="flex items-center justify-center gap-0.5 md:gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3 h-3 md:w-3.5 md:h-3.5 ${i < product.rating ? "fill-primary text-primary" : "fill-muted text-muted-foreground"}`} />
                      ))}
      ...
      <AnimatePresence>
        {toasts.length > 0 && (
          <div className="fixed bottom-6 right-6 z-[200] flex flex-col gap-3">
            {toasts.map((toast) => (
              <motion.div key={toast.id} initial={{ opacity: 0, x: 100, scale: 0.9 }} animate={{ opacity: 1, x: 0, scale: 1 }}
                exit={{ opacity: 0, x: 100, scale: 0.9 }} transition={{ type: "spring", damping: 20, stiffness: 300 }}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg min-w-[280px] ${
                  toast.type === 'success' ? 'bg-green-50 border border-green-200 text-green-800' :
                  toast.type === 'error' ? 'bg-red-50 border border-red-200 text-red-800' : 'bg-blue-50 border border-blue-200 text-blue-800'
                }`}>
                <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center ${
                  toast.type === 'success' ? 'bg-green-500' : toast.type === 'error' ? 'bg-red-500' : 'bg-blue-500'
                }`}>
                  {toast.type === 'success' ? <Check className="w-4 h-4 text-white" /> :
                   toast.type === 'error' ? <X className="w-4 h-4 text-white" /> : <Heart className="w-3 h-3 text-white" />}
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium">{toast.message}</p>
                  {toast.productTitle && <p className="text-xs opacity-75 truncate max-w-[200px]">{toast.productTitle}</p>}
                </div>
                <button onClick={() => setToasts((prev) => prev.filter((t) => t.id !== toast.id))} className="flex-shrink-0 p-1 hover:bg-black/10 rounded transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </motion.div>
            ))}
...
export default SilverSwingJhoola;
